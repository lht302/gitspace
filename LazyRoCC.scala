...
class  ComparatorExample(opcodes: OpcodeSet)(implicit p: Parameters) extends LazyRoCC(opcodes) {
  override lazy val module = new ComparatorExampleModuleImp(this)
}

class ComparatorExampleModuleImp(outer: ComparatorExample)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
    with HasCoreParameters {
  val a = Reg(chiselTypeOf(io.cmd.bits.rs1))
  val b = Reg(chiselTypeOf(io.cmd.bits.rs2))
  val c = Reg(chiselTypeOf(io.cmd.bits.rs1))
  val d = Reg(chiselTypeOf(io.cmd.bits.rs2))
  val rs1mid = Reg(chiselTypeOf(io.cmd.bits.rs1))
  val rs2mid= Reg(chiselTypeOf(io.cmd.bits.rs2))
  val req_rd = Reg(chiselTypeOf(io.resp.bits.rd))
  val result1 = Reg(chiselTypeOf(io.cmd.bits.rs2))
  val result2 = Reg(chiselTypeOf(io.cmd.bits.rs2))
  val result = Reg(chiselTypeOf(io.cmd.bits.rs2))
  val funct = Reg(chiselTypeOf(io.cmd.bits.inst.funct))
  ...
  val s_idle :: s_acq :: s_comp :: s_resp :: Nil = Enum(4)
  val state = RegInit(s_idle)
  val gt = a>b
  val gt1 = c>d
  val gt2 = result1>result2
  dontTouch(a)
  dontTouch(b)
  dontTouch(c)
  dontTouch(d)
  ...
  dontTouch(rs2mid)
  io.cmd.ready := (state === s_idle)
when(io.cmd.fire()) {funct := io.cmd.bits.inst.funct}

switch(state){
  is(s_idle){
    when(io.cmd.fire()){
    state := s_acq
    rs1mid := io.cmd.bits.rs1
    rs2mid := io.cmd.bits.rs2
    req_rd := io.cmd.bits.inst.rd
  }
  }
  is(s_acq){
  ...
  is(s_comp){
    state := s_resp
    when(comp1 === 1.U){
      when(gt===1.U) {result1 := a}
     .otherwise {result1 := b}
    }.elsewhen(comp2 === 1.U){
     when(gt1===1.U) {result2 := c}
     .otherwise {result2 := d}
   }.elsewhen(comp3 === 1.U){
     when(gt2===1.U) {result := result1}
     .otherwise {result := result2}
   }
  }
  is(s_resp){ 
    when (io.resp.fire()) { state := s_idle } 
  ...
  //when (io.resp.fire()) { state := s_idle }
  io.resp.valid := Mux(state === s_resp,1.U,0.U)
  io.resp.bits.rd := req_rd
